...
- LEGO LOGO + CLEAN ALPHA: rebranded the central logo + sidebar emblem to a LEGO-brick render of chatmaroc.ai. Production-grade transparency: regenerated the logo on a PURE BLACK matte, then `scripts/extract_logo_alpha.py` extracts a clean alpha (border-connected flood fill protects interior brick seams + luminance-feathered edges + unpremultiply to remove black spill). Result: `chatmaroc-lego.png` (full, transparent) for the welcome screen and `chatmaroc-lego-icon.png` (emblem square) for the sidebar. Verified 0.0% whitish soft-edge pixels → zero edge bleeding, blends flawlessly into the cyberpunk background. (Preview only.)
